...
public sealed class MySqlQueryServerCommandExecutor:IQueryCommandExecutor
{
    ...
    public ITextProtocolHandler TextProtocolHandler { get; }
    public int MySqlEncoding { get; }
    private int _currntSequenceId;

    public MySqlQueryServerCommandExecutor(MySqlQueryServerCommandPacket mySqlQueryServerCommandPacket,ConnectionSession connectionSession,ITextProtocolHandlerFactory textProtocolHandlerFactory)
    {
        ...
        TextProtocolHandler=isMultiCommands
            ? new MySqlMultiCommandHandler()
            : textProtocolHandlerFactory.Create(DatabaseTypeEnum.MySQL, sql, sqlCommand, connectionSession);
    ...
    {
        var responseHeader = TextProtocolHandler.Execute();
